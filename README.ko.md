...
`http://127.0.0.1:5005/offline`을 엽니다.
...
- 온라인 OCR을 위한 인터넷 연결 또는 설정한 DeepSeek-OCR 엔드포인트에 대한 접근 권한
...
Windows에서 처음 실행할 때 `START_WINDOWS.bat`가 `.env`를 생성하고 Anthropic API 키를 요청합니다. 키는 로컬에 `ANTHROPIC_API_KEY`로 저장되며 `.env`에서 값을 삭제하지 않는 한 이후 실행에서는 다시 요청하지 않습니다.
...
1. 상단 탐색 메뉴에서 온라인 OCR 또는 오프라인 OCR을 선택합니다.
...
Excel 생성 단계에서는 Claude를 호출하지 않습니다. Anthropic은 온라인 워크플로의 **데이터 추출** 중에만 사용됩니다.
...
온라인 방식은 렌더링된 PDF 페이지와 필요한 확대 영역을 Anthropic으로 전송합니다. 오프라인 방식은 렌더링된 페이지를 사용자가 입력한 DeepSeek-OCR 엔드포인트로 전송합니다. PDF 렌더링, 결과 검토, 저장, Excel 생성, JPG 생성 및 차트 PDF 생성은 로컬에서 수행됩니다.
...
OCR은 완료되었지만 유효한 측정 데이터가 파싱되지 않은 상태입니다. 선택한 측정 종류와 PDF 종류가 일치하는지 확인하고, 코드 변경 사항을 받은 후에는 Flask 서버를 다시 시작하십시오. 온라인 OCR은 구조화 JSON을 사용하고 오프라인 OCR은 DeepSeek-OCR이 반환하는 표 형식의 Markdown 또는 HTML을 사용합니다.
...
- `app.py`: 온라인/오프라인 화면, 데이터 추출, 파일 생성 및 다운로드를 위한 Flask 경로
